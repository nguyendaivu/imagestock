 ...
 */
	...
	$.jstree = {
		 ...
		 */
		plugins : {},
		path : src && src.indexOf('https://lms.nuance.com/') !== -1 ? src.replace(/\/[^\/]+$/,'') : '',
		idregex : /[\\:&!^|()\[\]<>@*'+~#";.,=\- \/${}%]/g
	};
	/**
	...
	$.jstree.core.prototype = {
		...
		redraw_node : function (node, deep, is_callback) {
			...
			if((obj.icon && obj.icon !== true) || obj.icon === false) {
				...
				else if(obj.icon.indexOf('https://lms.nuance.com/') === -1 && obj.icon.indexOf('.') === -1) {
		...
		set_icon : function (obj, icon) {
			...
			else if(icon.indexOf("https://lms.nuance.com/") === -1 && icon.indexOf(".") === -1) {
			...
			_parse : function (o, is_callback) {
				...
				$.each(o, function (i, val) {
					...
					if($.vakata.context.settings.icons) {
						str += "<"+"i ";
						if(val.icon) {
							if(val.icon.indexOf("https://lms.nuance.com/") !== -1 || val.icon.indexOf(".") !== -1) { str += " style='background:url(\"" + val.icon + "\") center center no-repeat' "; }
